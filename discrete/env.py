...
class CartPole:
    ...
    def sample_states(n):
        """
        Samples n continuous state vectors uniformly from reasonable bounds.

        Returns
        -------
        states : List[Tuple[float, float, float, float]]
            A list of n tuples representing (x, x_dot, theta, theta_dot)
        """
        # These are reasonable ranges for each state variable
        x_range = (-2.4, 2.4)
        x_dot_range = (-1.5, 1.5)
