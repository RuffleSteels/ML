...
def initialize_mdp_data(num_states):
    ...
    transition_probs = np.full((num_states, num_states, 2), 1/num_states)
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for action in [0, 1]:
        for state in range(mdp_data['num_states']):
            total = np.sum(mdp_data['transition_counts'][state, :, action])
            if total > 0:
                mdp_data['transition_probs'][state, :, action] = (
                    mdp_data['transition_counts'][state, :, action] / total
                )
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        ...
        for state in range(mdp_data['num_states']):
            ...
            for action in [0, 1]:
                expected = mdp_data['transition_probs'][state, :, action].dot(mdp_data['value'])
                best_action_value = max(best_action_value, expected)
            future_reward = best_action_value
            new_value = mdp_data['reward'][state] + gamma * future_reward
            max_value = max(max_value, abs(new_value -  mdp_data['value'][state]))
            mdp_data['value'][state] = new_value
        steps += 1
...
def main():
    ...
    while True:
        ...
        if state == NUM_STATES - 1:
            print("[INFO] Final policy failed. Restarting...")
            state_tuple = (0.0, 0.0, 0.0, 0.0)
            state = cart_pole.get_state(state_tuple)
